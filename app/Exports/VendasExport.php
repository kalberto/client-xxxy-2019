...
class VendasExport implements FromCollection,WithColumnFormatting, WithMapping,WithHeadings, ShouldAutoSize
{
	protected $vendas, $is_predio;
    ...
    */
	...
	{
		if($venda->is_empreendimento){
			$this->is_predio = false;
			$local = $this->map_local(false,$venda->predio_id);
		}else{
			$local = $this->map_local(true,$venda->predio_id);
		}
		$prospeccao = Prospeccao::find($venda->codigo);
		$auditoria = Auditoria::where('codigo',$venda->codigo)->latest()->first();
		$portado = '';
		$portados = '';
		$size = 0;
		...
		return [
			...
			isset($auditoria->tipo_nome) ? $auditoria->tipo_nome : 'Input',
			$venda->campanha,
	...
	public function map_local($is_predio, $predio_id){
		if($is_predio){
